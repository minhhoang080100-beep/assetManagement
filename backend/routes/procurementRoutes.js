...
import { createProcurementSchema, updateProcurementSchema } from '../helpers/schemas.js';
...
// POST /api/procurements/:id/import — Nhập kho từ phiếu hoàn tất
router.post('/:id/import', authMiddleware, authorizeRoles('ADMIN'), async (req, res, next) => {
  try {
    ...
    for (const item of proc.items) {
      for (let i = 0; i < item.quantity; i++) {
        ...
        const newEq = new Equipment({
          code,
          name: item.name,
          specs: `Theo phiếu ĐNMS ${proc._id.toString().slice(-6)}`,
          ...
          history: [{
            ...
            type: 'Nhập kho',
            description: `Nhập kho từ phiếu mua sắm ${proc.title}. Thực hiện bởi: ${req.user.username}`
          }]
        });
        await newEq.save();
        importedCount++;
